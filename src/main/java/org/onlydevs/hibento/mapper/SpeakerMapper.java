...
public class SpeakerMapper {
  ...
  private List<ExternalLinkDto> extractExternalLink(List<SpeakerExternalLink> links) {
    return links.stream()
        .map(l -> new ExternalLinkDto(l.getLinkType().toString(), l.getUrl()))
        .toList();
  }
  ...
  private List<EventSessionDto> extractEventSession(List<EventSessionSpeaker> sessionSpeakers) {
    return sessionSpeakers.stream()
        .map(
            s ->
                new EventSessionDto(
                    s.getEventSession().getId(),
                    s.getEventSession().getTitle(),
                    s.getEventSession().getEvent().getTitle(),
                    s.getEventSession().getStartTime(),
